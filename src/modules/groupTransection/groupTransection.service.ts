...
const getGroupStatus = async ({
    ...
    try {
        ...
        // Current effective expense = what you paid - settlements received + settlements paid
        // This represents your actual current financial contribution
        const currentEffectiveExpense = userTotalPaid - totalSettlementsReceived + totalSettlementsPaid;
        ...
        return {
            ...
            summary: {
                ...
                myExpensesAmount: parseInt(currentEffectiveExpense.toString()),
                originalPaidAmount: parseInt(userTotalPaid.toString()),
                settlementsReceived: parseInt(totalSettlementsReceived.toString()),
                settlementsPaid: parseInt(totalSettlementsPaid.toString()),
                netBalance: {
            ...
            categoryWise: Object.keys(categoryBreakdown).map(categoryName => {
                ...
                if (userTotalPaid > 0) {
                    categorySettlementImpact = (categoryBreakdown[categoryName].userPaid / userTotalPaid) * netSettlementImpact;
                }
            ...
            personWise: Object.keys(personBreakdown).map(email => {
                ...
                let settlementsToThisPerson = 0;   // Money user paid to this person
                
                if (group.settlements && group.settlements.length > 0) {
                    for (const settlement of group.settlements) {
                        // User received money from this person
                        if (settlement.toEmail === userEmail && settlement.fromEmail === email) {
                            settlementsFromThisPerson += settlement.amount;
                        }
                        // User paid money to this person
                        else if (settlement.fromEmail === userEmail && settlement.toEmail === email) {
                            settlementsToThisPerson += settlement.amount;
                        }
                    }
                }
...
const addGroupExpense = async ({ groupId, expenseData, user_id }: { groupId: string, expenseData: any, user_id: mongoose.Types.ObjectId | null }) => {
    ...
    try {
        ...
        // Normalize shareWith for convenience flags before validation
        let normalizedShareWith = shareWith;
        if (normalizedShareWith?.type === 'equal') {
            // If client passes members: ["all"], include owner + all group members
            if (Array.isArray(normalizedShareWith.members)
                && normalizedShareWith.members.length === 1
                && normalizedShareWith.members[0] === 'all') {
                normalizedShareWith = {
                    ...normalizedShareWith,
                    members: [group.ownerEmail, ...(group.groupMembers || [])]
                };
            }
            // If client passes includeOwner: true, ensure owner is part of members
            else if ((normalizedShareWith as any).includeOwner === true) {
                const set = new Set<string>(normalizedShareWith.members || []);
                if (group.ownerEmail) {
                    set.add(group.ownerEmail);
                }
                normalizedShareWith = {
                    ...normalizedShareWith,
        ...
        await validateShareWith(normalizedShareWith, totalExpenseAmount, group);
...
/* const updateGroupExpense = async ({ groupId, expenseId, expenseData, user_id }: { 
    ...
    try {
        ...
        const expenseIndex = group.groupExpenses?.findIndex((expense: any) => expense._id?.toString() === expenseId);

        if (expenseIndex === -1 || expenseIndex === undefined) {
            throw new Error('Expense not found');
        }

        const existingExpense = group.groupExpenses?.[expenseIndex] as any;

        if (!existingExpense) {
            throw new Error('Expense not found');
        }

        // Merge existing expense with updated data
        const {
            expenseDate,
            totalExpenseAmount,
            currency,
        ...
        // Validate updated expense data if critical fields are being changed
        if (totalExpenseAmount && totalExpenseAmount <= 0) {
            throw new Error('Total expense amount must be greater than 0');
        }

        if (paidBy) {
            await validatePaidBy(paidBy, updatedExpense.totalExpenseAmount, group);
            updatedExpense.paidBy = { ...paidBy, amount: updatedExpense.totalExpenseAmount };
        }

        if (shareWith) {
            await validateShareWith(shareWith, updatedExpense.totalExpenseAmount, group);
        }

        // Update the expense in the array
        group.groupExpenses![expenseIndex] = updatedExpense as any;
...
const updateGroupExpense = async ({
    ...
    try {
        ...
        if (expenseData.shareWith) {
            // Normalize shareWith for convenience flags before validation
            let normalizedShareWith = expenseData.shareWith;
            if (normalizedShareWith?.type === 'equal') {
                // If client passes members: ["all"], include owner + all group members
                if (Array.isArray(normalizedShareWith.members)
                    && normalizedShareWith.members.length === 1
            ...
            await validateShareWith(normalizedShareWith, existingExpense.totalExpenseAmount, group);
            existingExpense.shareWith = normalizedShareWith;
        }
...
const deleteGroupExpense = async ({ groupId, expenseId, user_id }: {
    ...
    try {
        ...
        if (!isOwner && !isMember) {
            throw new Error('You are not authorized to delete expenses in this group');
        }
        ...
        group.groupExpenses?.splice(expenseIndex, 1);
        ...
        await session.abortTransaction();
        console.error('Error in deleteGroupExpense service:', error.message);
        throw new Error(`Failed to delete group expense: ${error.message}`);
    } finally {
        session.endSession();
    }
};
